...
FMOD_RESULT F_CALLBACK finDelPlay(FMOD_CHANNEL *channel, FMOD_CHANNEL_CALLBACKTYPE type, void *commanddata1, void *commanddata2);
...
static void internalPlay(FMOD_SOUND* sound, float volume, bool repeat, int dest) // 0 para musica, 1 para otros
{
    ...
    g_fmodResult = FMOD_Channel_SetCallback(channel, finDelPlay);
    if (!ERRCHECK(g_fmodResult))
        return;
...
void SoundManager::PlayMusic(const string & fileName, bool repeat)
{
    ...
    if (sound)
    {
        ...
        //MusicFileInfo::ShowInfoFor(fileName);
    }
}
