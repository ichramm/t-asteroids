...
Nave::Nave(Objeto3d*obj,Vector posicion):Colisionable(obj,posicion)
{
    ...
    //cadenaParticulas = new CadenaParticulas(30);
    //cadenaParticulas->setElasticidad(100);
...
void Nave::tic(float dt){
    //if(vivo)
    ticVivo(dt);
    //cadenaParticulas->setPosParticula(0,this->getPosicion());
    ////cadenaParticulas->setPosParticula(cadenaParticulas->getCantParticulas()-1,this->getPosicion());
    quieroDispararPrimara=quieroDispararSecundaria=false;
}
